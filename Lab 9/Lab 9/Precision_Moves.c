...
#define RPM_SAMPLE_MS          40

#define TURN_KP                4
#define TURN_MIN_PWM           1000

#define STRAIGHT_PHASE_KP      3
#define STRAIGHT_PHASE_LIMIT   500
...
#define REF_STEPS              654    /* ≈ 40 cm in encoder steps              */
...
void Motor_Forward_RPM(uint16_t leftRPM, uint16_t rightRPM,
    ...
    int32_t movedL, movedR;
    int32_t deltaLSteps, deltaRSteps;
    uint32_t deltaLTime, deltaRTime;
    uint16_t actualLRPM, actualRRPM;
    int32_t errL, errR;
    int32_t integL = 0, integR = 0;
    int32_t leftPWM, rightPWM;
    int32_t phaseError, phaseCorrection;

    if(desiredLSteps <= 0 || desiredRSteps <= 0) return;
    if(leftRPM == 0 || rightRPM == 0) return;

    int32_t minSteps = (desiredLSteps < desiredRSteps) ? desiredLSteps : desiredRSteps;
    double  distScale   = ComputeDistScale(minSteps);
    ...
    while(1){
        ...
        if((movedL >= desiredLSteps) || (movedR >= desiredRSteps)) break;
        ...
        errL = (int32_t)leftRPM  - (int32_t)actualLRPM;
        errR = (int32_t)rightRPM - (int32_t)actualRRPM;

        integL += errL;
        integR += errR;

        if(integL >  MASTER_INT_LIMIT) integL =  MASTER_INT_LIMIT;
        if(integL < -MASTER_INT_LIMIT) integL = -MASTER_INT_LIMIT;
        if(integR >  MASTER_INT_LIMIT) integR =  MASTER_INT_LIMIT;
        if(integR < -MASTER_INT_LIMIT) integR = -MASTER_INT_LIMIT;

        leftPWM  += (MASTER_KP * errL) + (integL / MASTER_KI_DIV);
        rightPWM += (MASTER_KP * errR) + (integR / MASTER_KI_DIV);
        ...
        leftPWM  -= phaseCorrection;
        rightPWM += phaseCorrection;
